...
def kpi_card(icon, label, value, unit="", color=None, sublabel=""):
    ...
    return html.Div([
        html.Div(icon, style={
            ...
            "border": f"1.5px solid {accent}40",
        }),
        html.Div(label, style={
            "fontSize": "11px", "fontWeight": "600", "color": C["gray_mid"],
            "textTransform": "uppercase", "letterSpacing": "0.07em", "marginBottom": "8px",
        }),
        html.Div([
            html.Span(str(value), style={
                "fontSize": "30px", "fontWeight": "900", "color": accent, "lineHeight": "1",
            }),
            html.Span(f" {unit}", style={
                "fontSize": "13px", "fontWeight": "500", "color": C["gray_mid"],
            }) if unit else None,
        ]),
        html.Div(sublabel, style={
            "fontSize": "11px", "color": C["gray_mid"], "marginTop": "5px", "fontWeight": "400",
        }) if sublabel else None,
    ], style={
        "background": C["white"], "borderRadius": "25px", "padding": "22px 24px",
...
def _separator():
    return html.Div(style={
        ...
        "background": C["gray_light"], "marginBottom": "20px",
    })


def _panel_header(label):
    return html.Div([
        html.Div("● ", style={"color": C["orange"], "fontSize": "10px"}),
        html.Span(label, style={
            "fontSize": "13px", "fontWeight": "700", "color": C["gray_dark"],
        }),
    ], style={"display": "flex", "alignItems": "center", "gap": "6px", "marginBottom": "16px"})


# ── Layout ─────────────────────────────────────────────────────────────────────
app = dash.Dash(__name__, title="HR Copilot — Práxedes")
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #f5f5f5; color: #383838; font-family: 'Montserrat', sans-serif; }
...
def _empty_chart():
    return go.Figure().update_layout(
        ...
        annotations=[dict(
            text="Realiza una consulta para generar un gráfico",
            x=0.5, y=0.5, xref="paper", yref="paper",
            showarrow=False,
            font=dict(color=C["gray_light"], size=13, family=FONT),
        )],
    )
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div(style={
                ...
                "borderRadius": "50%", "background": C["orange"], "opacity": "0.85",
            }),
            html.Div(style={
                "position": "absolute", "top": "10px", "right": "280px",
                "width": "60px", "height": "60px",
                "borderRadius": "50%", "background": C["orange"], "opacity": "0.5",
            }),
            html.Div(style={
                "position": "absolute", "top": "-20px", "right": "20px",
                "width": "90px", "height": "90px",
                "borderRadius": "50%", "background": C["gray_light"], "opacity": "0.6",
            }),
            # Brand
            html.Div([
                html.Div([
                    html.Span("práxedes", style={
                        "fontFamily": FONT, "fontSize": "22px", "fontWeight": "900",
                        "color": C["white"], "letterSpacing": "-0.02em",
                    }),
                ], style={"marginBottom": "4px"}),
                html.Div([
                    html.Span("HR", style={
                        "fontFamily": FONT, "fontSize": "28px", "fontWeight": "900",
                        "color": C["white"],
                    }),
                    html.Span(" Copilot", style={
                        "fontFamily": FONT, "fontSize": "28px", "fontWeight": "400",
                        "color": C["orange_light"],
                    }),
                ]),
                html.Div("Análisis de People Analytics con IA generativa", style={
                    "fontSize": "12px", "color": "#ffffff99", "fontWeight": "400",
                    "marginTop": "4px",
                }),
            ], style={"position": "relative", "zIndex": "1"}),
        ], style={
            "background": C["gray_dark"], "padding": "28px 36px",
            "position": "relative", "overflow": "hidden",
            "borderRadius": "0 0 25px 25px", "marginBottom": "24px",
        }),

        # ── Role selector ──────────────────────────────────────────────────────
        html.Div([
            html.Span("Vista como:", style={
                "fontSize": "11px", "fontWeight": "600", "color": C["gray_mid"],
            ...
            dcc.Dropdown(
                ...
                value="hr_admin",
                clearable=False,
                style={"width": "290px", "fontFamily": FONT, "fontSize": "12px"},
            ),
            html.Div(id="rls-badge"),
        ], style={
        ...
        # ── KPI Cards ──────────────────────────────────────────────────────────
        html.Div([
            kpi_card("👥", "Headcount",      f"{kpis.get('headcount', 0):,}", "employees",
                     C["orange"],  "Workforce total"),
            kpi_card("📉", "Attrition rate", kpis.get("attrition", 0),       "%",
                     C["danger"],  "Employees who left"),
            kpi_card("💰", "Avg. income",    f"${kpis.get('avg_income', 0):,}", "/ mo",
                     C["info"],    "Monthly average"),
            kpi_card("⏱",  "Overtime rate",  kpis.get("overtime_pct", 0),    "%",
                     C["warning"], "Working overtime"),
            kpi_card("⭐", "Avg. performance", kpis.get("avg_perf", 0),      "/ 4",
                     C["success"], "Performance rating"),
            kpi_card("😊", "Job satisfaction", kpis.get("avg_sat", 0),       "/ 4",
                     C["orange"],  "Satisfaction score"),
        ], style={"display": "flex", "gap": "16px", "padding": "0 28px 24px", "flexWrap": "wrap"}),
        ...
        html.Div([
            ...
            # Left: chat panel
            html.Div([
                _panel_header("Consulta inteligente"),

                # Suggested questions
                html.Div("Preguntas sugeridas:", style={
                    "fontSize": "10px", "fontWeight": "600", "color": C["gray_mid"],
                    "textTransform": "uppercase", "letterSpacing": "0.07em",
                    "marginBottom": "10px",
                }),
                html.Div([
                    html.Button(q,
                        ...
                        style={
                            ...
                            "borderRadius": "20px", "color": C["gray_dark"],
                ...
                ], style={"display": "flex", "gap": "7px", "flexWrap": "wrap", "marginBottom": "18px"}),

                # Input bar
                html.Div([
                    dcc.Input(
                        id="input-question", type="text", debounce=False, n_submit=0,
                        placeholder="Escribe tu pregunta sobre los datos de RRHH...",
                        style={
                            "flex": "1", "background": C["white"],
                            "border": f"2px solid {C['gray_light']}",
                            "borderRadius": "25px", "color": C["gray_dark"],
                            "fontSize": "13px", "padding": "11px 20px",
                            "fontFamily": FONT, "fontWeight": "500",
                        },
                    ),
                    html.Button("Consultar →", id="btn-ask", n_clicks=0, style={
                        "background": C["orange"], "border": "none",
                        "borderRadius": "25px", "color": C["white"],
                ...
                html.Div(id="output-sql-block",   style={"marginBottom": "14px"}),
                html.Div(id="output-token-block", style={"marginBottom": "14px"}),
                html.Div(id="output-table-block"),
                html.Div(id="output-error-block"),

            ], style={
                "background": C["white"], "borderRadius": "25px", "padding": "24px",
                "flex": "1", "minWidth": "0", "boxShadow": "0 2px 10px rgba(0,0,0,0.07)",
            }),
            ...
            html.Div([
                ...
                "background": C["white"], "borderRadius": "25px", "padding": "24px",
                "flex": "1.2", "minWidth": "0", "boxShadow": "0 2px 10px rgba(0,0,0,0.07)",
            }),
        ...
        html.Div([
            _separator(),
            html.Span(
                "HR Copilot · Práxedes S.A.S. · Powered by Vanna AI + Gemini 2.5 Flash",
                style={"fontSize": "11px", "color": C["gray_mid"], "fontWeight": "500"},
            ),
        ], style={"padding": "0 28px 24px", "textAlign": "center"}),

    ], style={"background": C["gray_bg"], "minHeight": "100vh", "fontFamily": FONT})
